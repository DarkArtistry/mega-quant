  ...
  async swap(params: SwapParams): Promise<SwapResult> {
    ...
    try {
      ...
      if (this.quoterVersion === 2) {
        ...
        const result = await this.quoterContract.quoteExactInputSingle.staticCall(params)
      ...
      console.log(`[UniswapV3] Expected output: ${formatUnits(amountOutQuote, tokenOutInfo.decimals)} ${tokenOutInfo.symbol}`)
      ...
      console.log(`[UniswapV3] Actual output: ${formatUnits(actualAmountOut, tokenOutInfo.decimals)} ${tokenOutInfo.symbol}`)

      // 10. Calculate gas cost
      ...
      // 11. Record trade in database (non-blocking)
      await this.recordTrade({
        ...
        token_out_amount: formatUnits(actualAmountOut, tokenOutInfo.decimals),
        gas_used: gasUsed,
        gas_price_gwei: gasPriceGwei,
        gas_cost_usd: gasCostUsd
      })

      // Build explorer URL (reuse chainConfig from above)
      ...
      return {
        ...
        amountOut: formatUnits(actualAmountOut, tokenOutInfo.decimals),
        gasUsed,
        gasCostUsd,
        timestamp: Date.now(),
        explorerUrl
