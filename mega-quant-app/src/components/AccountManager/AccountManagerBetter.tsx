import { IoSettings, IoWarning, IoLeaf, IoKey, IoEye, IoEyeOff, IoWifi, IoCheckmarkCircle } from 'react-icons/io5';
...
export const AccountManagerBetter: React.FC<AccountManagerProps> = ({
  isOpen,
  onClose,
  networkConfigs,
  onConfigsUpdate,
  ...
  console.log('🚀 AccountManagerBetter loaded! isOpen:', isOpen);
  ...
  const deriveFirstAccount = async (walletId: string, walletName: string) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        console.log(`<IoCheckmarkCircle /> Auto-derived first account: ${accountName}`);
      }
    } catch (error) {
  ...
  const handleAddAccount = async () => {
    ...
    try {
      ...
      if (data.success) {
        await refreshAccounts();
        await fetchHdWallet();
        console.log(`<IoCheckmarkCircle /> Added account: ${accountName} at index ${nextIndex}`);
      } else {
  ...
  const handleImportAccount = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setShowNewAccountKey(false);
        setViewState('main');
        console.log(`<IoCheckmarkCircle /> Imported account: ${newAccountName}`);
      } else {
                ...
                {!showMnemonic ? (
                  ...
                  <div style={{
                    ...
                    <div style={{ marginBottom: '1.5rem', color: '#FFA500', fontWeight: 700, fontSize: '1.2rem' }}>
                      <IoWarning /> IMPORTANT: Save Your Recovery Phrase
                    </div>
                    ...
                    >
                      {copiedMnemonic ? '✓ Copied to Clipboard!' : '📋 Copy to Clipboard'}
                    </button>
