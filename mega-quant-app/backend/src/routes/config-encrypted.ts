...
router.post('/api-config/update', deriveEncryptionKey, (req, res) => {
  try {
    const { alchemyAppId, alchemyApiKey, etherscanApiKey, coinMarketCapApiKey, oneInchApiKey } = req.body
    const db = getDatabase()
    const encryptionKey = (req as any).encryptionKey as Buffer
...
router.post('/accounts/add', deriveEncryptionKey, async (req, res) => {
  try {
    ...
    // CRITICAL FIX: Reload all accounts into memory so the new account is immediately available
    if (password) {
      try {
        ...
        console.warn(`⚠️  Failed to reload accounts into memory: ${loadError.message}`)
...
router.post('/accounts/delete', deriveEncryptionKey, async (req, res) => {
  try {
    ...
    // CRITICAL FIX: Reload all accounts into memory to remove the deleted account
    if (password) {
      try {
        const { loadAllAccounts } = await import('../services/load-all-accounts.js')
        const { accountKeyStore } = await import('../services/account-key-store.js')
