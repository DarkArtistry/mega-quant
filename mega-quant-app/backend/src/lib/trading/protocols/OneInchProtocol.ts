...
export class OneInchProtocol extends ProtocolProxy {
    ...
    try {
      ...
      if (quoteData.estimatedGas) {
        try {
          ...
          const gasCostWei = BigInt(quoteData.estimatedGas) * (gasPrice.gasPrice || 0n)
  ...
  async swap(params: SwapParams): Promise<SwapResult> {
    ...
    try {
      ...
      // 5. Approve token if not native ETH
      ...
      // 9. Parse output amount
      const amountOut = formatUnits(swapData.dstAmount, tokenOutInfo.decimals)
      console.log(`[OneInch] Actual output: ${amountOut} ${tokenOutInfo.symbol}`)
      ...
      try {
        ...
        const chainConfig = getChainConfig(this.chainName)
        const nativeTokenPrice = await priceService.getTokenPriceUSD(chainConfig.nativeCurrency.symbol)
        const gasCostEth = Number(formatUnits(BigInt(gasCostWei), 18))
        gasCostUsd = gasCostEth * nativeTokenPrice
      } catch (error) {
