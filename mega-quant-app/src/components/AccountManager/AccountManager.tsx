...
import { IoSettings, IoWarning, IoLeaf, IoKey, IoEye, IoEyeOff, IoWifi, IoCheckmarkCircle, IoClose } from 'react-icons/io5';
...
export const AccountManager: React.FC<AccountManagerProps> = ({
  ...
  // Initialize accounts and check for HD wallet when modal opens
  useEffect(() => {
    console.log('[AccountManager] useEffect triggered - isOpen:', isOpen, 'masterPassword exists:', !!masterPassword);
    if (isOpen && masterPassword) {
      // Always fetch fresh data from backend when modal opens
      console.log('[AccountManager] Fetching accounts and HD wallet...');
      refreshAccounts();
      fetchHdWallet();
    }
  ...
  const handleAddAccount = async () => {
    ...
    try {
      ...
      if (data.success) {
        await refreshAccounts();
        await fetchHdWallet();
        console.log(`✅ Added account: ${accountName} at index ${nextIndex}`);
      } else {
  ...
  const handleImportAccount = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setShowNewAccountKey(false);
        setViewState('main');
        console.log(`✅ Imported account: ${newAccountName}`);
      } else {
  ...
  const refreshAccounts = async () => {
    console.log('[AccountManager] refreshAccounts called, masterPassword exists:', !!masterPassword);
    if (!masterPassword) {
      console.warn('[AccountManager] No master password, skipping refresh');
      return;
    }
    ...
    try {
      ...
      const data = await response.json();
      console.log('[AccountManager] API response:', data);

      if (data.success) {
        console.log('[AccountManager] Setting accounts:', data.accounts?.length || 0, 'accounts');
        setAccountsList(data.accounts || []);
        onAccountsUpdate(data.accounts || []);
      } else {
        console.error('[AccountManager] API returned error:', data.error);
      }
    } catch (error) {
      console.error('[AccountManager] Failed to refresh accounts:', error);
    }
  };
      ...
      <div className="account-manager-modal" style={{ maxWidth: '900px' }}>
        ...
        <div className="modal-header">
          <h2 className="modal-title">
            <span className="modal-icon"><IoSettings /></span>
        ...
        <div className="modal-content">
          ...
          <details className="form-section" style={{ marginBottom: '2rem' }}>
            <summary style={{ cursor: 'pointer', fontWeight: 'bold', color: '#00ff9f', fontSize: '1.1rem' }}>
              <IoWifi /> Network Selection ({selectedNetworks.length} selected)
            </summary>
                    ...
                    {network.logoUrl ? (
                      <img
                        ...
                        onError={(e) => {
                          ...
                          iconSpan.className = 'network-icon';
                          iconSpan.textContent = network.icon;
                          target.parentElement?.insertBefore(iconSpan, target);
                        }}
                      />
                    ) : (
                          ...
                          >
                            {network.logoUrl ? (
                              <img
                                src={network.logoUrl}
                                alt={network.displayName}
                                className="config-network-logo"
                                onError={(e) => {
                                  // Fallback to emoji icon if image fails to load
                                  const target = e.target as HTMLImageElement;
                                  target.style.display = 'none';
                                  const iconSpan = document.createElement('span');
                                  iconSpan.className = 'config-network-icon';
                                  iconSpan.textContent = network.icon;
                                  target.parentElement?.insertBefore(iconSpan, target);
                                }}
                              />
          ...
          <div className="form-section">
              ...
              <>
                ...
                {hasHdWallet && hdWallet && (
                  <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}>
                      <span style={{ fontSize: '1.2rem' }}><IoLeaf /></span>
                ...
                <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem' }}>
                  {!hasHdWallet ? (
                    ...
                    >
                      <span style={{ fontSize: '1.5rem' }}><IoLeaf /></span>
                  ...
                  >
                    <span style={{ fontSize: "1.5rem" }}><IoKey /></span>
                        ...
                        <div className="account-item-header">
                          <div className="account-item-name">
                            <span className="account-icon">
                              {account.accountType === 'hd' ? <IoLeaf /> : <IoKey />}
                            </span>
                        ...
                        <div className="account-item-details">
                          ...
                          <div className="account-detail-row">
                            ...
                            <div className="detail-value-with-toggle">
                              ...
                              >
                                {showAccountKeys[account.id] ? <IoEye /> : <IoEyeOff />}
                              </button>
                            </div>
                          </div>
                        </div>
                ...
                {!showMnemonic ? (
                  <div className="account-add-card">
                    <div className="account-add-header">
                      <span className="account-icon"><IoLeaf /></span>
                  ...
                  <div style={{
                    ...
                    >
                      {copiedMnemonic ? <><IoCheckmarkCircle /> Copied to Clipboard!</> : '📋 Copy to Clipboard'}
                    </button>
                    ...
                    >
                      <IoCheckmarkCircle /> I've Saved My Recovery Phrase
                    </button>
                  </div>
                )}
              </div>
            ...
            {viewState === 'import-account' && (
              <div className="account-add-card">
                <div className="account-add-header">
                  <span className="account-icon"><IoKey /></span>
                ...
                <div className="account-input-group">
                  ...
                  <div className="input-with-toggle">
                    ...
                    >
                      {showNewAccountKey ? <IoEye /> : <IoEyeOff />}
                    </button>
                  </div>
          ...
          {error && (
            <div className="error-banner">
              <span className="error-icon"><IoWarning /></span>
