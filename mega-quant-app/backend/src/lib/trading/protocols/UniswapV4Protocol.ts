...
// Supports hooks and singleton architecture

import { Contract, parseUnits, formatUnits, MaxUint256, AbiCoder, ZeroAddress, solidityPacked, hexlify } from 'ethers'
...
export class UniswapV4Protocol extends ProtocolProxy {
  ...
  async swap(params: V4SwapParams): Promise<SwapResult> {
    ...
    try {
      ...
      // Use custom fee/tickSpacing if provided, otherwise use defaults
      ...
      console.log(`[UniswapV4] Expected output: ${quote.amountOut} ${tokenOutInfo.symbol}`)
      ...
      // 10. Encode Universal Router V4 swap command
      ...
      const inputs = [abiCoder.encode(['bytes', 'bytes[]'], [actions, swapParams])]

      // Step 5: Set deadline (20 seconds from now, never use block.timestamp or max uint256)
      ...
      console.log(`[UniswapV4] Commands length: ${commands.length}`)
      console.log(`[UniswapV4] Inputs: ${JSON.stringify(inputs)}`)
      console.log(`[UniswapV4] Inputs length: ${inputs.length}`)
      ...
      // 11. Execute swap via Universal Router
      // Use the execute(bytes,bytes[],uint256) overload explicitly
      const txOptions: any = {
      ...
      // 13. Get actual output amount from balances
      // Since we used TAKE_ALL, the output tokens should be in our wallet
      ...
      // For a more accurate calculation, we should track balance before and after
      // For now, we'll use the quote amount as an approximation
      const amountOutActual = quote.amountOut

      console.log(`[UniswapV4] Swap completed successfully!`)
      console.log(`[UniswapV4] Amount in: ${params.amountIn} ${params.tokenIn}`)
      console.log(`[UniswapV4] Amount out: ${amountOutActual} ${params.tokenOut}`)

      // 14. Calculate gas price in Gwei
      ...
      await this.recordTrade({
        ...
        token_out_amount: amountOutActual,
        gas_used: gasUsedNum,
        gas_price_gwei: gasPriceGwei,
        gas_cost_usd: gasCostUsd
      })

      // Build explorer URL (reuse chainConfig from line 264)
  ...
  async getQuote(params: QuoteParams & { fee?: number, tickSpacing?: number }): Promise<QuoteResult> {
    ...
    // We need to catch the revert and decode the quote from the error
    ...
    try {
      ...
      // Error structure: error.data contains the encoded quote
      if (!error.data) {
        console.error('❌ Quote failed: No revert data')
      ...
      try {
        ...
        console.error('❌ Failed to decode quote revert data:', decodeError.message)
    ...
    console.log(`✅ Quote: ${amountOut} ${params.tokenOut} (rate: ${exchangeRate.toFixed(6)}, fee: ${fee/10000}%)`)
  ...
  private async approveToken(tokenAddress: string, amount: bigint): Promise<void> {
    ...
    const walletAddress = await this.wallet.getAddress()
    const poolManagerAddress = await this.poolManagerContract.getAddress()

    // Check current allowance
    const currentAllowance = await tokenContract.allowance(walletAddress, poolManagerAddress)

    if (currentAllowance < amount) {
      console.log(`[UniswapV4] Approving ${tokenAddress} for PoolManager...`)
      const approveTx = await tokenContract.approve(poolManagerAddress, MaxUint256)
   ...
   * @param tokenAddress Token to approve
   * @param amount Amount to approve
   * @param routerAddress SwapRouter address
   */
  private async approveTokenForRouter(tokenAddress: string, amount: bigint, routerAddress: string): Promise<void> {
    const tokenContract = new Contract(tokenAddress, ERC20_ABI, this.wallet)
    const walletAddress = await this.wallet.getAddress()
    ...
    if (currentAllowance < amount) {
      console.log(`[UniswapV4] Approving ${tokenAddress} for SwapRouter...`)
      const approveTx = await tokenContract.approve(routerAddress, MaxUint256)
      await approveTx.wait()
      console.log('[UniswapV4] Token approved for SwapRouter')
    } else {
      console.log('[UniswapV4] Token already approved for SwapRouter')
    }
  }
  ...
  async getQuoteWithHooks(
    ...
    // For now, use the standard getQuote and note that hooks may affect actual execution
  ...
  private async approveTokenViaPermit2(tokenAddress: string, amount: bigint): Promise<void> {
    ...
    if (needsApproval) {
      ...
      // Using MaxUint256 and converting to uint160 range
