import { IoWarning, IoLockOpen, IoLockClosed, IoKey, IoEye, IoEyeOff, IoHourglass, IoSettings, IoInformationCircle, IoFlash, IoRocket, IoBarChart, IoWifi, IoCheckmarkCircle, IoCheckmark, IoEllipseOutline, IoClose } from 'react-icons/io5';
    ...
    <div className="setup-password-overlay">
      <div className="setup-password-modal">
        ...
        <div className="setup-header">
          <div className="setup-icon"><IoLockClosed /></div>
        ...
        <div className="setup-content">
          ...
          <div className="strength-indicators">
            ...
            <div className={`strength-item ${strength.hasLength ? 'valid' : ''}`}>
              <span className="check-icon">{strength.hasLength ? <IoCheckmark /> : <IoEllipseOutline />}</span>
              <span>At least 8 characters</span>
            </div>
            <div className={`strength-item ${strength.hasUppercase ? 'valid' : ''}`}>
              <span className="check-icon">{strength.hasUppercase ? <IoCheckmark /> : <IoEllipseOutline />}</span>
              <span>One uppercase letter (A-Z)</span>
            </div>
            <div className={`strength-item ${strength.hasLowercase ? 'valid' : ''}`}>
              <span className="check-icon">{strength.hasLowercase ? <IoCheckmark /> : <IoEllipseOutline />}</span>
              <span>One lowercase letter (a-z)</span>
            </div>
            <div className={`strength-item ${strength.hasNumber ? 'valid' : ''}`}>
              <span className="check-icon">{strength.hasNumber ? <IoCheckmark /> : <IoEllipseOutline />}</span>
              <span>One number (0-9)</span>
            </div>
            <div className={`strength-item ${strength.hasSpecial ? 'valid' : ''}`}>
              <span className="check-icon">{strength.hasSpecial ? <IoCheckmark /> : <IoEllipseOutline />}</span>
          ...
          <div className="input-group">
            ...
            {confirmPassword && doPasswordsMatch() && (
              <div className="password-match"><IoCheckmark /> Passwords match</div>
            )}
            {confirmPassword && !doPasswordsMatch() && (
              <div className="password-mismatch"><IoClose /> Passwords do not match</div>
            )}
          </div>
          ...
          <div className="warning-box">
            <div className="warning-icon"><IoWarning />️</div>
        ...
        <div className="setup-footer">
          ...
          >
            {isLoading ? (
              <>
                <span className="loading-spinner"><IoHourglass /></span>
                <span>CREATING SECURE VAULT...</span>
              </>
            ) : (
              <>
                <span className="lock-icon"><IoLockClosed /></span>
