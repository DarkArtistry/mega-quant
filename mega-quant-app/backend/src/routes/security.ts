...
router.post('/unlock', async (req, res) => {
  try {
    ...
    try {
      const { decryptAlchemyApiKey, decryptCoinMarketCapApiKey, decryptOneInchApiKey } = await import('../services/encryption-service.js')
    ...
    // Load custom RPC URLs and configure liveDataService
    try {
      const { loadDecryptedRpcConfigs } = await import('../services/encryption-service.js')
      const { liveDataService } = await import('../services/live-data.js')

      const { alchemyApiKey, customRpcUrls } = loadDecryptedRpcConfigs(password)

      // Configure liveDataService with decrypted RPC settings
      if (alchemyApiKey || Object.keys(customRpcUrls).length > 0) {
        await liveDataService.configureAlchemyFromClient(alchemyApiKey || '', customRpcUrls)
        console.log(`[Security] ✅ RPC endpoints configured (${Object.keys(customRpcUrls).length} custom RPCs)`)
      }
    } catch (error: any) {
      console.warn('[Security] Warning: Failed to load custom RPCs:', error.message)
      // Don't fail unlock if RPC configuration fails
    }
...
router.post('/change-password', async (req, res) => {
  try {
    ...
    // TODO: Re-encrypt all sensitive data with new password
    // This is critical for security - all encrypted data must be re-encrypted
    // Steps:
    // 1. Derive old encryption key from current password
    // 2. Decrypt all private keys and API keys with old key
    // 3. Derive new encryption key from new password
    // 4. Re-encrypt all data with new key
    // 5. Update database
    //
    // For now, we're updating the password but NOT re-encrypting data
    // This means old encrypted data will still use the old password's key
    // SECURITY WARNING: Implement re-encryption before production use
